...
void upsample_cpu_vectorized(float *in, int w, int h, int c, int batch, int stride, int forward, float scale, float *out, int vecsize)
{
    ...
    for(b = 0; b < batch; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h*stride; ++j){
                for(i = 0; i < w*stride; ++i){
                    int in_index = b*w*h*c + ((k/vecsize)*w*h + (j/stride)*w + i/stride)*vecsize + (k%vecsize);
                    int out_index = b*w*h*c*stride*stride + ((k/vecsize)*w*h*stride*stride + j*w*stride + i)*vecsize + (k%vecsize);
                    if(forward) out[out_index] = scale*in[in_index];
                    else in[in_index] += scale*out[out_index];
                }
            }
